...
import { Bold, Code, Italic, Mark, Underline } from "editorjs-viewer-native";
import { Break } from "src/components/Break";
import { cleanAndTrimString } from "src/helpers/utils";

// ? This default list should not be responsibility of this hook ?
const defaultListTags = ["b", "code", "i", "mark", "u", "span", "br"];
...
export const useParseHtmlTags = () => {
  ...
  const createElementByTagName = useCallback(
    (tagChildren: string | ReactNode, tagName: string): ReactNode => {
      return createElement(getComponentByName(tagName), null, tagChildren);
    },
    [getComponentByName]
  );
          ...
          {createElementByTagName(
            parseHtmlTag(
              ...
              tagText.length > 0
                ...
                : cleanAndTrimString(textAfterTag)
            ),
            tagName
          )}
          {/* {textAfterTag && parseHtmlTag(listOfTags, cleanAndTrimString(textAfterTag))} */}
        </Fragment>
      );
    },
